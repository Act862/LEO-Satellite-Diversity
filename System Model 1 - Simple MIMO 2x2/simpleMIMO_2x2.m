...
startTime = datetime(2020,1,11,19,0,0);
stopTime = startTime + minutes(20);
sampleTime = 10;
...
hold off;
grid on;
title("Received E_b/N_o from both satellites (only DL)");
...
hold off;
grid on;
title('Received E_b/N_o from both links');
legend('Link 1', 'Link 2');
ylabel('E_b/N_o (dB)');
xlabel('Simulation Time (datetime)');
axis tight;
...
title('E_b/N_0 for path 1 comparison with full-path and DL');
...
grid on;
axis tight;
title("Received E_b/N_o on DL only + p618 losses");
ylabel('E_b/N_o (dB)');
xlabel('Simulation Time (datetime)');
...
grid on;
axis tight;
title("Received E_b/N_o from both links + p618 losses");
legend('Link 1', 'Link 2');
ylabel('E_b/N_o (dB)');
xlabel('Simulation Time (datetime)');
...
figure;
plot(time1,atmo_losses1,time2,atmo_losses2,'LineWidth',1.5);
legend('Link 1', 'Link 2');
grid on;
axis tight;
...
K = 10;
L = 2; % diversity order
ebnr_mrc_clean = path_ebnr_mrc(~isnan(path_ebnr_mrc) & path_ebnr_mrc ~= -Inf & path_ebnr_mrc ~= Inf);
ebnr_sc_clean = path_ebnr_sc(~isnan(path_ebnr_sc) & path_ebnr_sc ~= -Inf & path_ebnr_sc ~= Inf);
...
berTheory_mrc = berfading(ebnr_mrc_clean,"qam",M,L,K);
berTheory_sc = berfading(ebnr_sc_clean,"qam",M,L,K);

% semilogy(test_ebnr,berEst,'*'); 
% hold on;
semilogy(ebnr_mrc_clean,berTheory_mrc,'-*','LineWidth',1.5);
hold on;
semilogy(ebnr_sc_clean,berTheory_sc,'-^','LineWidth',1);
%   QPSK
M = 4;
% k = log2(M);
...
% end
% EbNoVec_theory = -10:30;
berTheory_mrc = berfading(ebnr_mrc_clean,"psk",M,L,K);
berTheory_sc = berfading(ebnr_sc_clean,"psk",M,L,K);
...
% semilogy(test_ebnr,berEst,'*'); hold on;
semilogy(ebnr_mrc_clean,berTheory_mrc,'-*','LineWidth',1.5); 
semilogy(ebnr_sc_clean,berTheory_sc,'-^','LineWidth',1); hold off;

grid on;
xlabel('Eb/No (dB)');
ylabel('Bit Error Rate');
title("BER Comparison between Different Modulations");
% legend('8-QAM (Simulation)','8-QAM (Theoretical)',...
%     'QPSK (Simulation)', 'QPSK (Theoretical)',...
%     '16-QAM (Simulation)','16-QAM (Theoretical)');
legend('8-QAM (MRC)','8-QAM (SC)',...
   'QPSK (MRC)','QPSK (SC)',...
    '16-QAM (MRC)','16-QAM (SC)');
axis tight;
