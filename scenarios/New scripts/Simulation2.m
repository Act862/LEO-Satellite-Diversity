...
for k=1:length(t)
    ...
    max_ebnr = -Inf;
    for s=1:length(sc.Satellites)
        %   each satellite has a different channel, thus a different
        %   channel coefficient
        K = 5;                    % Rician K-factor (linear, not dB)
        phi = 2*pi*rand;          % random LOS phase
        LOS = sqrt(K/(K+1));        % LOS component amplitude
        sigma = sqrt(1/(2*(K+1)));
        X = randn;
        Y = randn;
        h = LOS*exp(1j*phi) + sigma*(X + 1j*Y);
        
        ebno_nom_dB = ebno(links(s), t(k));                 % nominal Eb/N0 in dB
        ebnr = 10*log10(abs(h)^2) + ebno_nom_dB;            % faded Eb/N0 in dB
        % ebnr = ebno_nom_dB;            % unfaded Eb/N0 in dB
